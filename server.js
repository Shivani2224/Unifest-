...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        const result = await db.run('INSERT INTO users (name, email, password, department, year, role) VALUES (?, ?, ?, ?, ?, ?)', [name, email, password, department, year, role]);
...
app.post('/api/events', async (req, res) => {
    ...
    const db = await dbPromise;
    const result = await db.run('INSERT INTO events (name, description, venue, date, created_by) VALUES (?, ?, ?, ?, ?)', [name, description, venue, date, 1] // Mocked admin ID
    );
...
app.post('/api/registrations', async (req, res) => {
    ...
    const result = await db.run('INSERT INTO registrations (user_id, event_id, status) VALUES (?, ?, ?)', [user_id, event_id, 'pending']);
